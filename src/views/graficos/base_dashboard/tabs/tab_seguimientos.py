...
def render(tab2_data):
    ...
    with col1:
        with st.container(border=True):
            ...
            donut_chart_fig = charts_rodamientos.create_recaudo_donut_chart(conteo_estados_donut, estado_seleccionado="TODOS", show_center_text=False)
            if donut_chart_fig:
                donut_chart_fig.update_layout(height=420, margin=dict(t=30, b=10, l=10, r=10), legend=dict(orientation="h", yanchor="bottom", y=1.02, xanchor="center", x=0.5))
                # AGREGAMOS UN KEY ÚNICO
                st.plotly_chart(donut_chart_fig, use_container_width=True, key="chart_recaudo_general")
            else:
                st.info("Sin datos.")
    ...
    with col2:
        with st.container(border=True):
            ...
            sunburst_todos_fig = charts_rodamientos.create_nested_pie_chart(grouped_data_todos, conteo_data_todos, height=420)
            if sunburst_todos_fig:
                sunburst_todos_fig.update_layout(margin=dict(t=30, b=10, l=10, r=10))
                # AGREGAMOS UN KEY ÚNICO
                st.plotly_chart(sunburst_todos_fig, use_container_width=True, key="chart_gestion_global")
            else:
                st.info("Sin datos.")

    # --- COLUMNA 3: SOLO PAGO Y SIN PAGO ---
    with col3:
        # 1. CON PAGO
        with st.container(border=True):
            ...
            sunburst_pago_fig = charts_rodamientos.create_nested_pie_chart(grouped_data_pago, conteo_data_pago, height=180)
            if sunburst_pago_fig:
                sunburst_pago_fig.update_layout(margin=dict(t=20, b=10, l=10, r=10))
                # AGREGAMOS UN KEY ÚNICO
                st.plotly_chart(sunburst_pago_fig, use_container_width=True, key="chart_detalle_con_pago")
            else:
                st.write("Sin datos.")
        ...
        with st.container(border=True):
            ...
            detalle_sin_pago_fig = charts_rodamientos.create_nested_pie_chart(grouped_data_sin_pago, conteo_data_sin_pago, height=180)
            if detalle_sin_pago_fig:
                detalle_sin_pago_fig.update_layout(margin=dict(t=20, b=10, l=10, r=10))
                # AGREGAMOS UN KEY ÚNICO
                st.plotly_chart(detalle_sin_pago_fig, use_container_width=True, key="chart_detalle_sin_pago")
            else:
                st.write("Sin datos.")
    ...
    if df_completo is not None and not df_completo.empty:
        with st.expander("Filtros de Búsqueda (Gestión)", expanded=False):
            col_f1, col_f2, col_f3, col_f4 = st.columns(4)

            with col_f1:
                st.write("**Estado de Pago**")
                opciones_pago_gral = ['PAGO', 'SIN PAGO', 'ANTICIPADO'] 
                with st.popover("Seleccionar...", use_container_width=True):
                    ...
                    for opt in opciones_pago_gral:
                        if f"pago_gral_{opt}" not in st.session_state: st.session_state[f"pago_gral_{opt}"] = True
            ...
            with col_f2:
                ...
                with st.popover("Seleccionar...", use_container_width=True):
                    ...
                    for opt in opciones_gestion_gral:
                        if f"gest_gral_{opt}" not in st.session_state: st.session_state[f"gest_gral_{opt}"] = True
            ...
            with col_f3:
                ...
                with st.popover("Seleccionar Cargos...", use_container_width=True):
                    ...
                    for cargo in cargos_disponibles:
                        if f"cargo_{cargo}" not in st.session_state: st.session_state[f"cargo_{cargo}"] = True 
            ...
            with col_f4:
                ...
                with st.popover("Seleccionar Exclusión...", use_container_width=True):
                    ...
                    st.divider()
                    for cargo in cargos_disponibles:
                        if f"exclude_cargo_{cargo}" not in st.session_state: st.session_state[f"exclude_cargo_{cargo}"] = False
        ...
        if seleccion_pago_gral: df_filtrado = df_filtrado[df_filtrado['Estado_Pago'].isin(seleccion_pago_gral)]
        if seleccion_gestion_gral: df_filtrado = df_filtrado[df_filtrado['Estado_Gestion'].isin(seleccion_gestion_gral)]
        if filtro_cargos: df_filtrado = df_filtrado[df_filtrado['Cargo_Usuario'].isin(filtro_cargos)]
        ...
        with st.expander("Personalizar Columnas de la Tabla"):
            columnas_seleccionadas = st.multiselect(
                "Columnas visibles:", options=todas_las_columnas_disponibles, default=columnas_por_defecto,
                key="multi_cols_gestion_gral", label_visibility="collapsed"
            )
        ...
        if not columnas_seleccionadas:
            ...
            st.data_editor(df_filtrado[columnas_a_mostrar], use_container_width=True, hide_index=True, disabled=True, key="editor_busqueda_detallada", height=400)
        else:
    ...
    with st.container(border=True):
        ...
        if agg_rodamiento is not None and not agg_rodamiento.empty:
            ...
            if fig_rodamiento:
                ...
                # AGREGAMOS UN KEY ÚNICO AQUÍ TAMBIÉN POR PRECAUCIÓN
                st.plotly_chart(fig_rodamiento, use_container_width=True, key="chart_barras_rodamiento")
        else:
    ...
    if df_processed_cartera is not None and not df_processed_cartera.empty:
        with st.expander("Filtros de Búsqueda (Rodamientos)", expanded=False):
                ...
                if agg_rodamiento is not None and not agg_rodamiento.empty:
                    ...
                    with st.popover("Seleccionar...", use_container_width=True):
                        ...
                        for opt in rodamiento_options:
                            if f"rod_{opt}" not in st.session_state: st.session_state[f"rod_{opt}"] = True
            ...
            with col_f2:
                ...
                with st.popover("Seleccionar...", use_container_width=True):
                    ...
                    for opt in gestion_options:
                        if f"gestion_rod_{opt}" not in st.session_state: st.session_state[f"gestion_rod_{opt}"] = True
            ...
            with col_f3:
                st.write("**Pago**")
                pago_options = ['PAGO', 'SIN PAGO', 'ANTICIPADO'] 
                with st.popover("Seleccionar...", use_container_width=True):
                    ...
                    for opt in pago_options:
                        if f"pago_rod_{opt}" not in st.session_state: st.session_state[f"pago_rod_{opt}"] = True
        ...
        with st.expander("Personalizar Columnas (Rodamientos)"):
            columnas_seleccionadas = st.multiselect(
                "Columnas visibles:", options=columnas_disponibles,
                ...
                key="multi_cols_rodamientos", label_visibility="collapsed"
            )
        ...
        if not columnas_seleccionadas:
            ...
            st.data_editor(df_tabla[columnas_seleccionadas], use_container_width=True, hide_index=True, disabled=True, key="editor_detalle_rodamiento", height=400)
        else:
